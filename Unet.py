...
class CustomDataset(Dataset):
    def __init__(self, image_dir):
        ...
        self.images = np.load(image_dir + r"/signals.npy")
        self.scatters = np.load(image_dir + r"/scatters.npy")
        self.x, self.y = np.meshgrid(np.arange(256), np.arange(256))
    ...
    def __getitem__(self, idx):
        ...
        for point in self.scatters[idx]:
            if point[0] == 0.0:
                break
            distances = (self.x - point[0])**2 + (self.y - point[1])**2
            mask[distances < 25] = 1
